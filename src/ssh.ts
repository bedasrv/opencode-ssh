...
export type ProcessRunner = {
  run(file: string, args: string[], options?: { timeout?: number; stdio?: "pipe" | "ignore" }): Promise<{ stdout: string; stderr: string; code: number }>
  start?(file: string, args: string[], options?: { timeout?: number }): Promise<unknown>
}
...
export type ConnectionState = { host: string; socketPath: string; configPath?: string; mode?: "shell" | "workspace"; remotePath?: string; localDirectory?: string }
...
export function validateRemoteCwd(cwd: string): void {
  if (!cwd.startsWith("/") || /[\x00-\x1f\x7f]/.test(cwd) || cwd.split("/").some((part) => part === "..")) throw new Error("Invalid remote workspace directory")
}
 ...
 */
export function wrapRemoteCommand(socket: string, host: string, command: string, configPath?: string, remoteCwd?: string): string {
...
type WrapRecord = { original: string; wrapped: string; socket: string; host: string; configPath?: string; remoteCwd?: string }
const wrapRecords = new WeakMap<object, WrapRecord>()

function translateWorkspaceCommand(command: string, localDirectory: string | undefined, remotePath: string | undefined): string {
  if (!localDirectory || !remotePath) return command
  const localRoot = localDirectory.length > 1 && localDirectory.endsWith("/") ? localDirectory.slice(0, -1) : localDirectory
  if (localRoot === "/") return command
...
export function transformShellExecuteBefore(
  ...
  if (event.tool !== "shell" && event.tool !== "bash") {
    if ((LOCAL_WORKSPACE_TOOLS as readonly string[]).includes(event.tool) && state.mode !== "workspace") {
...
export function workspaceSystemMessage(host: string, remotePath: string): string {
...
export class SshConnections {
  ...
  connect(sessionID: string, host: string): Promise<ConnectionState> {
    ...
    return this.runExclusive(sessionID, async () => {
      if (this.states.get(sessionID)?.mode === "workspace") await this.disconnectUnlocked(sessionID)
      const state = await this.connectUnlocked(sessionID, host)
      state.mode = "shell"
      delete state.remotePath; delete state.localDirectory
      return state
    })
  }

  connectWorkspace(sessionID: string, host: string, remotePath: string, localDirectory: string): Promise<ConnectionState> {
    if (this.closing) return Promise.reject(new Error("opencode-ssh is shutting down; start a new session to reconnect"))
    validateHost(host); validateRemoteCwd(remotePath)
    return this.runExclusive(sessionID, async () => {
